...
def log(message, level=None, name=None, filename=None):
    ...
    elif config.settings.log_console:
...
def create_hdf5(dir=None,filename=None,overwrite_hdf5=False):
    ...
    assert '.h5' in filename and filename.split('.')[-1] == 'h5', 'hdf5 filename extension must be "h5"'
    ...
    if not os.path.exists(hdf5_save_path):
        ...
        store = pd.HDFStore(hdf5_save_path)
        store.close()
        log('Existing {} hdf5 store in dir: has been overwritten.'.format(hdf5_save_path))
    else:
        log('Using existing {} hdf5 store.'.format(hdf5_save_path))

    return hdf5_save_path

def df_to_hdf5(data=None,key=None,overwrite_key=False,dir=None,filename=None,overwrite_hdf5=False):
    """
    ...
    None
    """
    hdf5_save_path = create_hdf5(dir=dir,filename=filename,overwrite_hdf5=overwrite_hdf5)
    ...
    if not ''.join(['/',key]) in store.keys():
        ...
        store.close()
        data.to_hdf(hdf5_save_path, key=key, mode='a', format='table')
        log('Existing {} overwritten in {} hdf5 store.'.format(key,hdf5_save_path))

    else:
        store.close()
        log('Key {} already exists in {} hdf5 store. Set to overwrite_key = True to replace.'.format(key,hdf5_save_path))
...
def hdf5_to_df(dir=None,filename=None,key=None):
    ...
    assert '.h5' in filename and filename.split('.')[-1] == 'h5', 'hdf5 filename extension must be "h5"'
    assert os.path.exists(hdf5_load_path), 'Unable to find directory or file: {}'.format(hdf5_load_path)

    with pd.HDFStore(hdf5_load_path) as store:

        log('Successfully read store: {} with the following keys: {}'.format(hdf5_load_path,store.keys()))
        try:
            df = store[key]
