...
def plot_net(nodes, edges, x_col=None, y_col=None, from_col=None,
    ...
    # TODO: make assertion instead
    if y_max - y_min <= 0 or x_max - x_min <= 0:
        raise_with_traceback(ValueError(
                                        'difference between min and max x '
                                        'and or y resulted in a negative '
                                        'value or 0'))
...
def _recursive_category_gen(col, num_bins):
    ...
    # base case catch
    if num_bins == 0:
        raise_with_traceback(ValueError('Unable to perform qcut to 0 bins.'))
...
def _prep_edges(edges, nodes, from_col, to_col,
    ...
    '{} or {} columns were not found in nodes columns'.format(x_col, y_col))
