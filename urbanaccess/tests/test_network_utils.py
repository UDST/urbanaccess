import pytest
import pandas as pd
from urbanaccess import network_utils
...
def test_nearest_neighbor(osm_nodes_df, transit_nodes_df,
    ...
    transit_nodes_df['nearest_osm_node'] = network_utils.nearest_neighbor(
        osm_nodes_df[['x', 'y']], transit_nodes_df[['x', 'y']])
...
def test_connector_edges(osm_nodes_df, transit_nodes_df,
    ...
    net_connector_edges = network_utils.connector_edges(
        osm_nodes_df, transit_nodes_df, travel_speed_mph=3)
    ...
    col_order = ['from', 'to', 'weight', 'net_type']
    expected_connector_edge_df = expected_connector_edge_df[col_order]
    net_connector_edges = net_connector_edges[col_order]
