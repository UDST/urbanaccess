...
# set the number of pandana networks in memory to arbitrary 40 for removing low connectivity nodes
reserve_num_graphs(40)


def ua_network_from_bbox(lat_min=None, lng_min=None, lat_max=None, lng_max=None, bbox=None, network_type='walk',
                      timeout=180, memory=None, max_query_area_size=50*1000*50*1000, remove_lcn=True):
    """
    ...
    # returned osm data allows travel in both directions so that all edges in integrated network are all one way edges
    two_way=False
    ...
    if remove_lcn:
        ...
        lcn = pandana_net.low_connectivity_nodes(impedance=10000, count=10, imp_name='distance')
