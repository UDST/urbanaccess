...
class urbanaccess_network(object):
    """
    A urbanaccess object of dataframes representing the components of a graph network
...
def nearest_neighbor(df1=None, df2=None):
    """
    For a dataframe of xy coordinates find the nearest xy coordinates in a subsequent dataframe
    ...
    df2 : pandas.DataFrame
        dataframe of records with xy coordinates for which to find the nearest record in df1 for
    Returns
    -------
...
def integrate_network(urbanaccess_network=None,headways=False,urbanaccess_gtfsfeeds_df=None,headway_statistic='mean'):
    """
    Create an integrated network comprised of transit and OSM nodes and edges by connecting the
    transit network with the osm network. travel time is in units of minutes
    ...
    headways : bool, optional
        if true, route stop level headways calculated in a previous step will be applied to the osm to transit connector
        edge travel time weights as an approximate measure of average passenger transit stop waiting time.
    urbanaccess_gtfsfeeds_df : object, optional
        required if headways is true; the gtfsfeeds_df object that holds the corresponding headways and stops dataframes
...
def add_headway_impedance(ped_to_transit_edges_df=None,headways_df=None,headway_statistic='mean'):
    """
    Add route stop level headways to the osm to transit connector travel time weight column
...
def route_id_to_node(stops_df=None,edges_w_routes=None):
    """
...
def connector_edges(osm_nodes=None,transit_nodes=None,travel_speed_mph=3):
    """
    Generate the connector edges between the osm and transit edges and weight by travel time
    ...
    travel_speed_mph : int, optional
        travel speed to use to calculate travel time across a distance on a edge. units are in miles per hour (MPH)
    ...
    transit_nodes['nearest_osm_node'] = nearest_neighbor(osm_nodes[['x', 'y']],transit_nodes[['x', 'y']])
    ...
    for transit_node_id, row in transit_nodes.iterrows():
        ...
        net_connector_edges.append((transit_node_id, osm_node_id, time_transit_to_ped, net_type))
        # make the edge bi-directional
        net_type = 'osm to transit'
        net_connector_edges.append((osm_node_id, transit_node_id, time_ped_to_transit, net_type))

    net_connector_edges = pd.DataFrame(net_connector_edges, columns=["from", "to", "weight","net_type"])
...
def format_pandana_edges_nodes(edge_df=None,node_df=None):
    """
    Perform final formatting on nodes and edge dataframes to prepare them for use in Pandana.
    Formatting mainly consists of creating a unique node id and edge from and to id that is integer
    based per Pandana requirements.
    ...
    """
    # pandana requires ids that are integer: for nodes - make it the index, for edges make it the from and to columns
    node_df['id_int'] = range(1, len(node_df) + 1)

    edge_df.rename(columns={'id': 'edge_id'},inplace=True)
    tmp = pd.merge(edge_df, node_df[['id','id_int']], left_on='from', right_on='id', sort=False, copy=False,how='left')
    tmp['from_int'] = tmp['id_int']
    tmp.drop(['id_int','id'], axis=1, inplace=True)
    edge_df_wnumericid = pd.merge(tmp, node_df[['id','id_int']], left_on='to', right_on='id', sort=False, copy=False,how='left')
...
def save_network(urbanaccess_network=None,dir=config.settings.data_folder,filename=None,overwrite_key=False,overwrite_hdf5=False):
    """
    ...
    df_to_hdf5(data=urbanaccess_network.net_edges,key='edges',overwrite_key=overwrite_key,dir=dir,filename=filename,overwrite_hdf5=overwrite_hdf5)
    df_to_hdf5(data=urbanaccess_network.net_nodes,key='nodes',overwrite_key=overwrite_key,dir=dir,filename=filename,overwrite_hdf5=overwrite_hdf5)

def load_network(dir=config.settings.data_folder,filename=None):
    """
    Read data from a hdf5 file to a urbanaccess_network object
